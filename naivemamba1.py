...
class S6(nnx.Module):
    def __init__(self, rngs:nnx.Rngs, D, N:int=64, R:int=1, complex_ssm:bool=True, use_euler_barB_approx:bool=True, use_log_A_stability_trick:bool=True, use_bf16=False):
    ...
    def __call__(self, x):
        ...
        self.state_caches = xs[:,0,...]

        return ys if not self.complex_ssm else ys.real / 2


class Mamba(nnx.Module):
    def __init__(self, rngs:nnx.Rngs, in_features:int, out_features:int, D:int, N:int=64, R:int=1, causal_conv_kernel_size:int=4, use_euler_barB_approx:bool=True, complex_ssm:bool=False, use_log_A_stability_trick:bool=True, bf16=False):
        ...
        self.conv = nnx.Conv(in_features=D, out_features=D, kernel_size=causal_conv_kernel_size, feature_group_count=D, padding="CAUSAL", use_bias=False, rngs=rngs, dtype=dtype)
        self.sigma = nnx.silu
        self.s6 = S6(rngs, D, N=N, R=R, use_euler_barB_approx=use_euler_barB_approx, complex_ssm=complex_ssm, use_log_A_stability_trick=use_log_A_stability_trick, use_bf16=bf16)
