...
#include "selective_scan_bwd_support.cuh"
...
template<> __device__ __forceinline__ complex_t conj<complex_t>(complex_t x) { return thrust::conj(x); }
...
void selective_scan_bwd_kernel(SSMParamsBwd params) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            if constexpr (!kIsComplex) {
                ...
                const float initial_state =
                    reinterpret_cast<float *>(params.initial_x_ptr)[
                        (batch_id * params.dim + dim_id) * params.dstate
                        + state_idx];
                scan_t running_prefix = chunk > 0 && threadIdx.x % 32 == 0
                    ? x[(chunk - 1) * params.dstate + state_idx]
                    : make_float2(1.f, initial_state);
                SSMScanPrefixCallbackOp<weight_t> prefix_op(running_prefix);
                typename Ktraits::BlockScanT(smem_scan).InclusiveScan(
                    thread_data, thread_data, SSMScanOp<weight_t>(), prefix_op
                );
                const float final_state_cotangent =
                    reinterpret_cast<float *>(params.dfinal_x_ptr)[
                        (batch_id * params.dim + dim_id) * params.dstate
                        + state_idx];
                scan_t running_postfix =
                    chunk < params.n_chunks - 1 && threadIdx.x % 32 == 0
                    ? smem_running_postfix[state_idx]
                    : make_float2(1.f, final_state_cotangent);
    ...
    for (int state_idx = threadIdx.x; state_idx < params.dstate; state_idx += blockDim.x) {
        float *dinitial_x =
            reinterpret_cast<float *>(params.dinitial_x_ptr)
            + (batch_id * params.dim + dim_id) * params.dstate;
        dinitial_x[state_idx] =
            smem_delta_a[state_idx] * smem_running_postfix[state_idx].y;
...
cudaError_t selective_scan_bwd_launch(
    SSMParamsBwd &params, cudaStream_t stream) {
    return BOOL_SWITCH(
        params.seqlen % (kNThreads * kNItems) == 0,
        kIsEvenLen,
        [&] {
            ...
            constexpr int kSmemSize =
                ...
                + (kNThreads + 4 * MAX_DSTATE)
                    * sizeof(typename Ktraits::weight_t);
...
inline cudaError_t selective_scan_bwd_cuda(
    SSMParamsBwd &params, cudaStream_t stream) {
    if (params.seqlen <= 128) {
        return selective_scan_bwd_launch<32, 4>(params, stream);
    }
    if (params.seqlen <= 256) {
        return selective_scan_bwd_launch<32, 8>(params, stream);
    }
    if (params.seqlen <= 512) {
        return selective_scan_bwd_launch<32, 16>(params, stream);
    }
    if (params.seqlen <= 1024) {
        return selective_scan_bwd_launch<64, 16>(params, stream);
    }
    return selective_scan_bwd_launch<128, 16>(params, stream);
}
